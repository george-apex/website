...
const ANIMATION_CONFIG = {
  timing: {
    firstCycleIdleDuration: 0.1,
    subsequentIdleDuration: 0.3,
    morphDuration: 1.5,
...
export type AlicePhase = 'idle' | 'morphing_to_model' | 'showing_model' | 'morphing_to_idle'

interface SculptureLinesProps {
  onPhaseChange?: (phase: AlicePhase) => void
}

function SculptureLines({ onPhaseChange }: SculptureLinesProps) {
  const groupRef = useRef<THREE.Group>(null)
  const linesDataRef = useRef<LineData[]>([])
  const [phase, setPhase] = useState<AlicePhase>('idle')
  const prevPhaseRef = useRef<AlicePhase>('idle')
...
interface AliceParticlesProps {
  className?: string
  onPhaseChange?: (phase: AlicePhase) => void
}

export function AliceParticles({ className = '', onPhaseChange }: AliceParticlesProps) {
  return (
